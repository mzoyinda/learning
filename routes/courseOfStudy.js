...
courseOfStudyRoute.get("/courseOfStudy", async (req, res, next) => {
  ...
  await databaseConn.dbVar.query(
    `SELECT courseTitle, roleTitle, roleDescription, applicableAreas FROM courses JOIN rolescourses ON courses.courseId = rolescourses.courseID JOIN roles ON rolescourses.roleID = roles.roleID`,
    function (err, result, fields) {
      if (err) throw err;

      for (let index = 0; index < result.length; index++) {
        if (
          courseOfStudy.replace(/\s+/g, "").toLowerCase() ===
          result[index].courseTitle.replace(/\s+/g, "").toLowerCase()
        ) {
